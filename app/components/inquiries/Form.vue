...
<template>
  <form @submit.prevent="onSubmit"
    ...
    <div class="form-group my-2">
      ...
      <textarea @input="textEvent" v-model="text" class="form-control shadow-sm" :class="{'_clear' : textOk}" rows="8" required placeholder="お問い合わせ内容をご記入ください">お問い合わせ内容をご記入ください</textarea>
      <div class="_alert">{{alertText}}</div>
    </div>
    <div class="form-group my-2 d-flex justify-content-center">
      <label for="registerUserId" class="form-label">送信した内容を後で確認できるようにする：</label>
      <input v-model="registerUserId" id="registerUserId" type="checkbox" class="form-check-input mx-2 shadow-sm">
    </div>
