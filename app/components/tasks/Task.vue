...
const isBeforeYesterday = useIsBeforeYesterday(new Date(task.value.dueDate))
...
<template>
  <div v-if="isTaskShow" class="_task_container" :class="{'_simple_container' : isSimpleDisplay, '_completed_task_container' : task.completed , '_fadeout_toggle_completed': toggleFadeout, '_fadein_toggle_completed': toggleFadein}">
    <div class="_task_box" :class="{'_completed_task_box': task.completed, '_delete_fadeout' : deleteFadeout}">
      <div class="_task_row_1 mt-2" :class="{'_simple_row_1' : isSimpleDisplay}">
        <div class="_title_parent d-flex justify-content-between align-items-start w-100">
          ...
          '_task_time_l' : task.dueStartTime && task.dueEndTime,
           '_simple_task_time' : isSimpleDisplay && !(task.isRecurring && !isBeforeYesterday)}">
            ...
            <div :class="{'_hidden' : !isSimpleDisplay || (task.isRecurring && !isBeforeYesterday)}" class="_simple_btns d-flex justify-content-around">
      ...
      <div class="_task_row_2 my-2 pt-2 d-flex justify-content-between" :class="{'_hidden' : isSimpleDisplay}">
        ...
        <div class="_task_btns d-flex justify-content-around" :class="{'_hidden' : task.isRecurring && !isBeforeYesterday}">
