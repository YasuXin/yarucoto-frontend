...
useHead({
  title: 'Yarucoto | Verify'
    })
const { login } = useAuth()

const config = useRuntimeConfig()
const baseUrl = config.public.apiBase
const postVerifyUrl = baseUrl + '/auth/signup/verify'
...
const onVerify = async (numbersText: string) => {
  ...
  try {
    ...
    if (resultSubmit.value === 'no-record') {
      ...
      modalMessage.value = '会員登録が完了しました'
...
const submitResult = ref<boolean>(false)
const onSubmit = async () => {
  modalMessage.value = '送信中...'
  modalShow.value = true
  try {
    submitResult.value = await login(email.value, password.value)
  } catch (error) {
    modalShow.value = false
...
<template>
  <div id="content" class="d-flex justify-content-center" :class="{'_hidden': !loginHidden}">
    <main>
      <h3 class="mt-3 mb-4 text-center">登録完了画面</h3>
      <form class="d-flex justify-content-center align-items-center flex-column">
        ...
        <ul id="verify-texts">
          <li>仮登録から30分以内に登録手続きを完了してください</li>
