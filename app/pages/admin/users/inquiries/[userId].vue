...
import type {AdminInquiry, AdminUser} from "~/interfaces";
...
useHead({
  title: "Yarucoto | Admin | Users | Inquiry"
})
...
const getInquiriesUrl = baseUrl + '/admin/users/inquiries/get/' + userId
...
const movePaginationEvent = (page: number) => {
  getUserAndInquiries(page)
  navigateTo({path: '/admin/users/inquiries/' + userId, query: {
      page: page,
...
const moveLastPaginationEvent = () => {
  getUserAndInquiries(totalPages.value)
  navigateTo({path: '/admin/users/inquiries/' + userId, query: {
...
const getUserAndInquiries = async (page: number) => {
  const params = {
    page: page,
    columnKeyword: columnKeyword.value ?? null,
    keyword: keyword.value ?? null,
...
const onSearch = async (params: SearchParams) => {
  ...
  await getUserAndInquiries(currentPage.value)
  navigateTo({path: '/admin/users/inquiries/' + userId, query: params as any})
  window.scrollTo(0, 0)
}
...
<template>
  <div id="main-child">
    <div id="main-title" class="d-flex justify-content-center">
      <h3>{{user?.username}}さんのお問い合わせ一覧</h3>
    </div>
